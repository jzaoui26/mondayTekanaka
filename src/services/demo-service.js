...
class DemoService {
  ...
  static async manageRowCSV(shortLivedToken, boardId, columnUniqueId, csvrow, columnsBoard)
  {
    ...
    // get itemId
    const columnId = DemoService.getColumnNameId(columnsBoard, columnName)

    // verify
    if(columnId == '') return false;

    // verify in map
    const itemId = await DemoService.getItemId(shortLivedToken, boardId, columnUniqueId, idUnique);
      ...
      try
      {
        fs.createReadStream(inputFile)
          ...
          .on('data', function(csvrow)
          {
            ...
            if(compteur >= 1)
            {
                DemoService.manageRowCSV(shortLivedToken, boardId, columnUniqueId, csvrow, columnsBoard);
            }
  ...
  static addScheduledTrigger(webhookUrl, hour, utcDaysDiff, timezone) {
    ...
    DemoService.fetchHook(webhookUrl);
